...
class Game:
    ...
    def mainloop(self, sf, door):
        while True:
            if self.running:
                for sprite in self.sprites:
                    sprite.move()
            else:
                door.open_door(self, tk=self.tk)
                time.sleep(0.7)
                sf.go(self, door, tk=self.tk)
                time.sleep(0.7)
                door.close_door(self, tk=self.tk)
                time.sleep(1.5)
                tkinter.messagebox.showinfo('Игра', 'ПОБЕДА')
                self.tk.destroy()
                break
...
class MovingPlatformSprite(Sprite):
    def __init__(self, game, photo_image, x, y, width, height, x_left, x_right):
        Sprite.__init__(self, game)
        self.photo_image = photo_image
        self.image = game.canvas.create_image(x, y, image=self.photo_image, anchor='nw')
        self.coordinates = Coords(x, y, x + width, y + height)
    ...
    def move(self):
        ...
        if self.coordinates.x1 <= self.x_left:
            self.x = 1
        elif self.coordinates.x2 >= self.x_right:
            self.x = -1
        self.coordinates = Coords(self.coordinates.x1 + self.x, self.coordinates.y1 , self.coordinates.x1 + self.x + self.width, self.coordinates.y2)
...
class StickFigureSprite(Sprite):
    ...
    def go(self, game, door, tk):
        game.canvas.itemconfig(self.image, image=PhotoImage(file='door2.gif'))
        tk.update()
...
class DoorSprite(Sprite):
    def __init__(self, game, photo_image1, photo_image2, x, y, width, height):
        Sprite.__init__(self, game)
        self.photo_close = photo_image1
        self.image_open = photo_image2
        self.image = game.canvas.create_image(x, y, image=self.photo_close, anchor='nw')
        self.coordinates = Coords(x, y, x + (width / 2), y + height)
        self.endgame = True

    def open_door(self, game, tk):
        self.game.canvas.itemconfig(self.image, image=self.image_open)
        tk.update()

    def close_door(self, game, tk):
        self.game.canvas.itemconfig(self.image, image=self.photo_close)
        tk.update()
...
def first_level():
    ...
    platform5 = MovingPlatformSprite(g, PhotoImage(file="platform2.gif"), 175, 350, 66, 10, 75, 425)
